...
namespace Exercise02 {
    internal class Program {
        ...
        private static void Exercise4(List<Book> books) {
            var farst = books.FirstOrDefault(b => b.Price >= 4000);
            if (farst is not null) {
                Console.WriteLine(farst.Title);
            }
        }

        private static void Exercise5(List<Book> books) {
            var num = books.Where(b => b.Price <= 4000).Max(b => b.Pages);
            Console.WriteLine(num);
        }

        private static void Exercise6(List<Book> books) {
            var upPrice = books.Where(b => b.Pages >= 400).OrderByDescending(b => b.Price);
            foreach (var item in upPrice) {
                Console.WriteLine("{0} {1}", item.Title, item.Price);
            }
        }

        private static void Exercise7(List<Book> books) {
            var selbook = books.Where(b => b.Title.Contains("C#") && b.Pages <= 500).Select(b => b.Title);
            foreach (var item in selbook) {
